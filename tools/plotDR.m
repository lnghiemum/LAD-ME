...
if strcmpi(morph,'disc'),
    for i=a:b,
        ...
        elseif d>1,
            ...
            set(gca,'xtick',[],'ytick',[]);
        elseif d==1;
% %             [MUHAT,SIGMAHAT] = normfit(class(:))
% %             t = -10*SIGMAHAT:.001:10*SIGMAHAT;
% %             s = normpdf(t,MUHAT,SIGMAHAT);
% %             plot(t,s,label); hold on;
%             subplot(b-a+1,1,i);
%             hist(class(:),10); 
%             if ismodel,
%                 xlabel(strcat(model,'-1'));
...
%             h = max(Y);
            colores = {'b','r','g','k','y'};
            for k=1:b,
%                 hdl = findobj(gca,'Type','patch');
%                 set(hdl,'FaceColor',colores{k},'EdgeColor','w')
