...
function [val]= valin_basic(X,Y,u,h,sigma,sigmag,sizes)
    ...
    auxmtx = setaux_v2(X,Y,h,p,V,D);
    val(1+1,:,:) = getSIRv2(u,auxmtx);
    val(1+2,:,:) = getSAVEv2(u,p,auxmtx);
    val(1+3,:,:) = getDRv2(u,p,auxmtx);
...
if comb<5000,
    ...
    val(comb+1,:,:) = getSIRv2(u,auxmtx);
    val(comb+2,:,:) = getSAVEv2(u,p,auxmtx);
    val(comb+3,:,:) = getDRv2(u,p,auxmtx);
    ...
    disp('WARNING: due to the size of p and u, a reduced number of candidates will be used for initialization');
    %----take the first u eigenvectors of marginal covariance matrix as directions---
    [diaguD,idx] = sort(diag(D),'descend'); 
    val(1,:,:) = V(:,idx(1:u));
    val(1+1,:,:) = getSIRv2(u,auxmtx);
